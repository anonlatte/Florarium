...
class SettingsFragment : Fragment() {
    ...
    private fun handleCommand(command: SettingsCommand) {
        val toastMessageRes = when (command) {
            SettingsCommand.ErrorTimeUpdated -> {
                R.string.content_setting_notifications_time_error
            }

            SettingsCommand.SuccessTimeUpdated -> {
                R.string.content_setting_notifications_time_updated
            }
        }
        Toast.makeText(
            requireContext(),
            getString(toastMessageRes),
            Toast.LENGTH_SHORT
        ).show()
    }
